...
func JDK_java_lang_System_initProperties(properties ObjectRef) ObjectRef {
	...
	paths := strings.Split(classpath, "|") //old: ":"
	var abs_paths []string
	for _, seg := range paths {
		if seg != "" {
			abs_path, _ := filepath.Abs(seg)
			abs_paths = append(abs_paths, strings.Replace(abs_path, "\\", "/", -1))
		}
	}
	classpath = strings.Join(abs_paths, "|") //old: ":"
	...
	m := map[string]string{
		...
		"os.name":    "Windows", //"os.name":    "Mac OS X", // FIXME
